...
<div class="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden select-none">
    ...
    <div class="p-6 md:p-8 flex flex-col md:flex-row justify-between gap-4 bg-slate-50/30 border-b border-slate-100">
        ...
        <form method="GET" action="{{ route('subadmin.konsultasi.index') }}" class="flex flex-wrap items-center gap-3 w-full">
            <div>
            ...
            @if(request()->filled('status') || request()->filled('search'))
                <a href="{{ route('subadmin.konsultasi.index') }}" class="bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold text-sm px-4 py-2.5 rounded-xl transition">Reset</a>
            @endif
        </form>
    </div>
        ...
        <table class="w-full text-left border-collapse">
            <thead class="bg-slate-50 border-b border-slate-100">
                <tr>
                    ...
                    <th class="px-5 py-4 text-left text-[10px] font-black uppercase tracking-widest text-slate-400">Topik Konsultasi</th>
            ...
            <tbody class="divide-y divide-slate-50">
                ...
                <tr class="hover:bg-slate-50/80 transition cursor-pointer group" onclick="window.location='{{ route('subadmin.konsultasi.show', $p->id_pendaftaran) }}'">
                    <td class="p-4">
