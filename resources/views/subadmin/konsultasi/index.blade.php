...
<div class="bg-white rounded-[2rem] border border-slate-100 p-8 shadow-sm select-none">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
            <h3 class="text-xl font-black text-slate-900 tracking-tight">Daftar Pengajuan Konsultasi</h3>
            <p class="text-xs text-slate-500 mt-1">Kelola dan pantau proses konsultasi K3 untuk mitra perusahaan.</p>
        </div>
        
        <!-- Filter Status & Search Form -->
        <form method="GET" action="{{ route('subadmin.konsultasi.index') }}" class="flex flex-wrap items-center gap-3 w-full md:w-auto">
            <div>
            ...
            <div class="relative flex-1 md:flex-initial">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari perusahaan / PIC..." class="border border-slate-200 rounded-xl pl-9 pr-4 py-2.5 text-xs bg-slate-50 font-medium text-slate-700 focus:outline-none focus:ring-4 focus:ring-cyan-500/10 w-full md:w-60">
                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                </span>
            </div>
            @if(request()->filled('status') || request()->filled('search'))
                <a href="{{ route('subadmin.konsultasi.index') }}" class="text-xs font-bold text-red-500 hover:text-red-700 transition">Reset</a>
            @endif
        </form>
    </div>
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-slate-50 border-b border-slate-100">
                    ...
                    <th class="p-4 text-xs font-black uppercase text-slate-500 tracking-wider">Topik Konsultasi</th>
                    <th class="p-4 text-xs font-black uppercase text-slate-500 tracking-wider">Tanggal Usulan</th>
            ...
            <tbody class="divide-y divide-slate-50">
                ...
                <tr class="hover:bg-slate-50/60 transition group">
                    ...
                    <td class="p-4 text-sm font-medium text-slate-700">
                        {{ $p->jadwal?->jenis?->nama ?? 'Konsultasi SMK3' }}
                    </td>
                    ...
                    <td class="p-4 text-right">
                        <a href="{{ route('subadmin.konsultasi.show', $p->id_pendaftaran) }}" class="inline-block bg-slate-900 hover:bg-slate-800 text-white text-[10px] font-black uppercase tracking-wider px-4 py-2.5 rounded-xl shadow-sm transition">
                            Kelola Proses
                        </a>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="5" class="p-12 text-center text-sm font-medium text-slate-400">Belum ada data pendaftaran konsultasi yang sesuai filter.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
