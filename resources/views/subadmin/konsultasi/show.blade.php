...
@section('title', 'Kelola Proses Konsultasi')
@section('page-title', 'Kelola Konsultasi: ' . ($pendaftaran->perusahaan?->nama ?? 'B2B Client'))

@section('content')

{{-- ── TOP BREADCRUMB & HEADER ─────────────────────────────── --}}
<div class="mb-8 flex items-center justify-between">
    <a href="{{ route('subadmin.konsultasi.index') }}" class="flex items-center gap-2 text-xs font-black text-slate-400 hover:text-slate-600 transition uppercase tracking-widest">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-[2rem] border border-slate-100 p-8 shadow-sm">
            <h3 class="text-base font-black text-slate-900 tracking-tight mb-8">Alur Tahapan Konsultasi</h3>

            @php
                $stages = [
                    'meninjau'            => ['Tinjauan Awal',        'Subadmin meninjau pengajuan & kebutuhan konsultasi.'],
                    'disetujui'           => ['Konfirmasi',            'Konsultasi disetujui, siap masuk tahap penjadwalan.'],
                    'dijadwalkan'         => ['Penjadwalan',           'Menentukan jadwal pertemuan & konsultan.'],
                    'menunggu_pelaksanaan' => ['Menunggu Pelaksanaan',  'Jadwal telah dikonfirmasi, menunggu sesi dilaksanakan.'],
                    'menunggu_pembayaran' => ['Tagihan Dikirim',       'Sesi selesai — tagihan dikirim ke klien.'],
                    'pembayaran_ditinjau' => ['Verifikasi Pembayaran', 'Bukti transfer diunggah klien, menunggu konfirmasi.'],
                    'selesai'             => ['Selesai',               'Seluruh proses konsultasi & pembayaran berhasil selesai.'],
                ];
                ...
                @if($currentStage === 'dibatalkan')
                    <div class="relative">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-400 mt-0.5">Pengajuan ini telah ditolak atau dibatalkan.</p>
                        </div>
                    </div>
                @endif
            </div>
        ...
        <div class="bg-white rounded-[2rem] border border-slate-100 p-8 shadow-sm">
            ...
            @if($pendaftaran->status_progres === 'meninjau')
                <div class="flex items-center gap-3 mb-6 pb-5 border-b border-slate-100">
                    ...
                    <div>
                        ...
                        <h3 class="text-base font-black text-slate-900">Tinjau Pengajuan Konsultasi</h3>
                    </div>
                </div>
                <p class="text-sm text-slate-600 leading-relaxed mb-6">
                    Harap tinjau kebutuhan konsultasi, profil perusahaan, dan usulan tanggal di panel kanan. Jika data dinilai sesuai, setujui permintaan ini untuk lanjut ke tahap penjadwalan.
                </p>
                <div class="flex gap-3">
                    <form action="{{ route('subadmin.konsultasi.confirm', $pendaftaran->id_pendaftaran) }}" method="POST" class="flex-1">
                        @csrf
                    ...
                    <form action="{{ route('subadmin.konsultasi.reject-payment', $pendaftaran->id_pendaftaran) }}" method="POST">
                        @csrf
                        <button type="submit"
                                onclick="return confirm('Tolak dan batalkan permintaan konsultasi ini?')"
            ...
            @if($pendaftaran->status_progres === 'disetujui')
                <div class="flex items-center gap-3 mb-6 pb-5 border-b border-slate-100">
                    ...
                    <div>
                        <p class="text-xs text-slate-400 font-semibold uppercase tracking-widest">Tindakan Diperlukan</p>
                        <h3 class="text-base font-black text-slate-900">Mulai Penjadwalan Sesi</h3>
                    </div>
                </div>
                <p class="text-sm text-slate-600 leading-relaxed mb-6">
                    Permintaan konsultasi telah disetujui. Lanjutkan ke tahap berikutnya untuk menentukan tanggal, waktu, mode pertemuan, dan konsultan pendamping.
                </p>
                <form action="{{ route('subadmin.konsultasi.start-scheduling', $pendaftaran->id_pendaftaran) }}" method="POST">
                    @csrf
                    <button type="submit" class="w-full bg-[#1E6B3D] hover:bg-[#24824A] text-white font-black text-sm px-6 py-4 rounded-2xl shadow-lg shadow-emerald-700/10 transition uppercase tracking-wider flex items-center justify-center gap-2">
            ...
            @if($pendaftaran->status_progres === 'dijadwalkan')
                <div class="flex items-center gap-3 mb-6 pb-5 border-b border-slate-100">
                    ...
                    <div>
                        <p class="text-xs text-slate-400 font-semibold uppercase tracking-widest">Tindakan Diperlukan</p>
                        <h3 class="text-base font-black text-slate-900">Tentukan Jadwal & Konsultan</h3>
                    </div>
                </div>
                <p class="text-sm text-slate-500 leading-relaxed mb-6">
                    Isi detail jadwal pertemuan dan pilih konsultan/pemateri yang akan mendampingi. Setelah disimpan, jadwal pertemuan akan dikonfirmasi dan status otomatis beralih ke tahap pelaksanaan.
                </p>

                <form action="{{ route('subadmin.konsultasi.schedule', $pendaftaran->id_pendaftaran) }}" method="POST" class="space-y-5">
                    @csrf

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            <label class="block text-xs font-bold text-slate-700 mb-1.5">Tanggal Pertemuan <span class="text-red-500">*</span></label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            <label class="block text-xs font-bold text-slate-700 mb-1.5">Mode Pertemuan <span class="text-red-500">*</span></label>
                            <select name="mode_pertemuan" id="mode_pertemuan" required
                                ...
                                <option value="online" {{ old('mode_pertemuan', $pendaftaran->mode_pertemuan) === 'online' ? 'selected' : '' }}>🌐 Online (Zoom/Meet)</option>
                                <option value="offline" {{ old('mode_pertemuan', $pendaftaran->mode_pertemuan) === 'offline' ? 'selected' : '' }}>🏢 Offline (Tatap Muka)</option>
                        ...
                        <div id="lokasi-container" class="{{ old('mode_pertemuan', $pendaftaran->mode_pertemuan) === 'offline' ? '' : 'hidden' }}">
                            <label class="block text-xs font-bold text-slate-700 mb-1.5">Lokasi Fisik Pertemuan <span class="text-red-500">*</span></label>
                            <input type="text" name="lokasi" id="input-lokasi"
                                   value="{{ old('lokasi', $pendaftaran->jadwal?->lokasi) }}"
                                   placeholder="Contoh: Kantor Mitra atau Ruang Meeting Veritas"
                                   class="w-full border border-slate-200 rounded-xl px-4 py-3 text-xs focus:ring-4 focus:ring-cyan-500/10 focus:outline-none focus:border-cyan-600 text-slate-700 font-semibold bg-slate-50">
                        </div>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-700 mb-3">Pilih Konsultan / Pemateri <span class="text-red-500">*</span></label>
            ...
            @if($pendaftaran->status_progres === 'menunggu_pelaksanaan')
                <div class="flex items-center gap-3 mb-6 pb-5 border-b border-slate-100">
                    ...
                    <div>
                        ...
                        <h3 class="text-base font-black text-slate-900">Ringkasan Jadwal Pertemuan</h3>
                    </div>
                </div>
                ...
                <div class="bg-gradient-to-br from-indigo-50 to-slate-50 rounded-2xl p-5 border border-indigo-100/60 mb-6">
                    <div class="grid grid-cols-2 gap-4 text-xs">
                        ...
                        <div>
                            <span class="text-slate-400 font-bold uppercase block mb-0.5">
                                @if($pendaftaran->mode_pertemuan === 'offline') Lokasi @else Link Meeting @endif
                            </span>
                    ...
                    @if($pendaftaran->jadwal?->pemateri && $pendaftaran->jadwal->pemateri->count() > 0)
                        <div class="pt-4 mt-4 border-t border-indigo-100/60">
                            <span class="text-slate-400 text-xs font-bold uppercase block mb-2">Konsultan Pendamping:</span>
                ...
                <div class="mt-6 pt-6 border-t border-slate-100">
                    <h4 class="text-sm font-black text-slate-800 tracking-tight mb-3">Buat Tagihan Pembayaran & Selesaikan Pemesanan</h4>
                    <p class="text-xs text-slate-500 leading-relaxed mb-4">
                        Setelah sesi konsultasi selesai dilaksanakan, masukkan total tagihan resmi untuk pemesanan layanan konsultasi ini. Setelah dikirim, invoice otomatis dikirim ke email klien dan status berubah ke "Menunggu Pembayaran".
                    </p>

                    <form action="{{ route('subadmin.konsultasi.finish', $pendaftaran->id_pendaftaran) }}" method="POST" class="space-y-4">
                        @csrf
                        <div>
                            <label class="block text-xs font-bold text-slate-700 mb-1.5">Total Nilai Tagihan Konsultasi (Rp) <span class="text-red-500">*</span></label>
                            <div class="relative">
                ...
                {{-- Form Edit Jadwal (tersembunyi, muncul saat "Ubah Jadwal" diklik) --}}
                <div id="edit-jadwal-form" class="hidden mt-6 pt-6 border-t border-slate-100">
                    <p class="text-xs font-bold text-slate-700 mb-4 uppercase tracking-wider">✏️ Edit Jadwal Pertemuan</p>
                    <form action="{{ route('subadmin.konsultasi.schedule', $pendaftaran->id_pendaftaran) }}" method="POST" class="space-y-4">
                        @csrf
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-600 mb-2">Konsultan / Pemateri *</label>
            ...
            @if($pendaftaran->status_progres === 'pembayaran_ditinjau')
                ...
                <div class="flex gap-3">
                    <form action="{{ route('subadmin.konsultasi.confirm-payment', $pendaftaran->id_pendaftaran) }}" method="POST" class="flex-1">
                        @csrf
                        <button type="submit"
                                onclick="return confirm('Konfirmasi pembayaran sebagai LUNAS dan tandai konsultasi selesai?')"
            ...
            @if($pendaftaran->status_progres === 'selesai')
                <div class="p-8 bg-gradient-to-br from-emerald-50 to-green-50 border border-emerald-200 rounded-2xl text-center space-y-4">
                    ...
                    <h4 class="text-base font-black text-emerald-800 uppercase tracking-wider">Konsultasi Selesai & Lunas</h4>
                    <p class="text-sm text-emerald-700 max-w-sm mx-auto leading-relaxed">
                        Seluruh proses — dari pendaftaran, penjadwalan, pelaksanaan, hingga pembayaran — telah berhasil diselesaikan.
                    </p>
                    <div class="pt-2 border-t border-emerald-200/60">
                        <span class="text-xs text-emerald-600 font-semibold">Total Nilai Konsultasi: </span>
            ...
            @if($pendaftaran->status_progres === 'dibatalkan')
                <div class="p-8 bg-rose-50 border border-rose-200 rounded-2xl text-center space-y-3">
                    ...
                    <p class="text-sm text-rose-700 max-w-sm mx-auto leading-relaxed">
                        Pengajuan layanan konsultasi ini telah ditolak atau dibatalkan dari sistem.
                    </p>
                </div>
            @endif
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-[2rem] border border-slate-100 p-6 shadow-sm">
            <h3 class="text-sm font-black text-slate-900 tracking-tight mb-5 flex items-center gap-2">
                <i class="fi fi-rr-building text-cyan-600"></i>
        ...
        <div class="bg-white rounded-[2rem] border border-slate-100 p-6 shadow-sm">
            ...
            <div class="space-y-4">
                @foreach([
                    ...
                    ['Mode Rencana', strtoupper($pendaftaran->mode_pertemuan ?? 'online')],
                ] as [$label, $value])
                <div>
                    <span class="text-[10px] text-slate-400 font-bold uppercase tracking-wider block mb-0.5">{{ $label }}</span>
