                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-6 bg-slate-50 border border-slate-200 rounded-2xl p-4">
                            <div class="relative w-20 h-20 bg-white rounded-xl border border-slate-200 overflow-hidden flex items-center justify-center flex-shrink-0">
                                @if($jadwal->foto)
                                    <img id="foto-preview" src="{{ asset('storage/' . $jadwal->foto) }}" alt="Preview" class="w-full h-full object-cover">
                                    <svg id="foto-placeholder" class="hidden w-8 h-8 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                @else
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-[10px] text-slate-400 mt-1">Format: JPG, JPEG, atau PNG. Maksimal 2MB. Biarkan kosong jika tidak ingin mengubah foto.</p>
                            </div>
                        </div>
...
function previewFoto(input) {
    ...
    if (input.files && input.files[0]) {
        ...
        @if($jadwal->foto)
            preview.src = "{{ asset('storage/' . $jadwal->foto) }}";
            preview.classList.remove('hidden');
            placeholder.classList.add('hidden');
        @else
            preview.src = '#';
