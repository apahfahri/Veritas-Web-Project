    ...
    <div class="bg-white p-8 rounded-xl shadow">
        ...
        <form method="POST" action="{{ route('admin.pelatihan.update', $layanan->id_layanan) }}" class="space-y-5">
            ...
            <div>
                ...
                <select name="id_kategori" required class="w-full border rounded-lg p-3 focus:ring-2 focus:ring-[#7d2ae7] focus:outline-none">
                    @foreach($kategoris as $k)
                        <option value="{{ $k->id_kategori }}" {{ old('id_kategori', $layanan->id_kategori) == $k->id_kategori ? 'selected' : '' }}>{{ $k->nama }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <select name="pemateri_ids[]" multiple class="w-full border rounded-lg p-3 focus:ring-2 focus:ring-[#7d2ae7] focus:outline-none h-32">
                    @foreach($pemateris as $p)
                        <option value="{{ $p->id_pemateri }}" 
                            {{ (collect(old('pemateri_ids', $layanan->pemateri->pluck('id_pemateri')))->contains($p->id_pemateri)) ? 'selected' : '' }}>
                            {{ $p->nama_lengkap }}
                        </option>
                    @endforeach
                </select>
