        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y">
                ...
                <tr class="hover:bg-gray-50">
                    <td class="p-4">
                        <div class="font-medium text-[#7d2ae7]">{{ $p->user?->nama }}</div>
                        <div class="text-xs text-gray-500">{{ $p->user?->email }}</div>
                    </td>
                    <td class="p-4 text-gray-600">{{ $p->layanan?->materi ?? '-' }}</td>
                    <td class="p-4 text-gray-500 whitespace-nowrap">{{ $p->tanggal_daftar ? $p->tanggal_daftar->format('d M Y') : '-' }}</td>
                    <td class="p-4">
                        ...
                        <span class="px-2 py-1 rounded-full text-xs {{ $c }}">{{ ucfirst(str_replace('_', ' ', $p->status_progres)) }}</span>
                    </td>
                    <td class="p-4">
                        @php $cb = match($p->status_bayar) { 'lunas' => 'bg-green-100 text-green-700', default => 'bg-gray-100 text-gray-600' }; @endphp
