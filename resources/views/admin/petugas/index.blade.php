        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-slate-50/50">
                    <th class="p-6 text-[10px] font-black uppercase tracking-widest text-slate-400 border-b border-slate-50">Nama & Identitas</th>
                    <th class="p-6 text-[10px] font-black uppercase tracking-widest text-slate-400 border-b border-slate-50">Kontak Detail</th>
                    <th class="p-6 text-[10px] font-black uppercase tracking-widest text-slate-400 border-b border-slate-50">Kompetensi Utama</th>
                ...
                <tr class="hover:bg-slate-50/50 transition-colors group">
                    <td class="p-6">
                        <div class="flex items-center gap-4">
                            ...
                            <div class="flex flex-col">
                                <span class="text-sm font-black text-slate-900 group-hover:text-indigo-600 transition">{{ $p->nama_lengkap }}</span>
                                <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">INSTRUKTUR KHUSUS</span>
                            </div>
                        </div>
                    </td>
                    <td class="p-6">
                        <div class="flex flex-col gap-1.5">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-xs font-bold text-slate-600 lowercase">{{ $p->email }}</span>
                            </div>
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-xs font-bold text-slate-600">{{ $p->no_hp ?? '-' }}</span>
                            </div>
                        </div>
                    </td>
                    <td class="p-6">
                        <p class="text-xs text-slate-500 font-bold leading-relaxed max-w-xs truncate">{{ $p->kompetensi ?? '-' }}</p>
                    </td>
                    <td class="p-6 text-center">
                        <div class="flex items-center justify-center gap-2">
                            ...
                            <form method="POST" action="{{ route('admin.petugas.destroy', $p->id_pemateri) }}"
                                   onsubmit="return confirm('Hapus pemateri {{ $p->nama_lengkap }}?')">
                                @csrf @method('DELETE')
                                <button type="submit"
